  ...
  private static final String PATH_DELIMITER = "/";
    ...
    public void registerKcRoutingHandler(final KcRoutingRecorder recorder,
        ...
        pathRedirectsMap.forEach((k, v) -> {
          Log.infof("Creating Redirect Routes: %s %s", k, v);
        ...
        pathPrefixesMap.forEach((k, v) -> {
          Log.infof("Creating Prefix Routes: %s %s", k, v);
          routes.produce(nonApplicationRootPathBuildItem.routeBuilder()
                  .orderedRoute(k + "/*", 1)
                  .handler(recorder.getHandler())
                  .build());
        ...
        pathFiltersMap.forEach((k, v) -> {
          Log.infof("Creating Filter Routes: %s %s", k, v);
          routes.produce(nonApplicationRootPathBuildItem.routeBuilder()
                  .orderedRoute(k, 1)
                  .handler(recorder.getHandler())
                  .build());
        ...
        pathBlocksMap.forEach((k, v) -> {
          Log.infof("Creating Block Routes: %s %s", k, v);
          routes.produce(nonApplicationRootPathBuildItem.routeBuilder()
                  .orderedRoute(k, 1)
                  .handler(recorder.getHandler())
                  .build());
        ...
        pathRecursiveBlocksMap.forEach((k, v) -> {
          Log.infof("Creating Recursive Block Routes: %s %s", k, v);
          routes.produce(nonApplicationRootPathBuildItem.routeBuilder()
                  .orderedRoute(k + "*", 1)
                  .handler(recorder.getHandler())
                  .build());
        ...
        pathAllowsMap.forEach((k, v) -> Log.infof("Creating Allow Rules: %s %s", k, v));
